    ...
    if ($result) { // Check if the query ran successfully
        if ($result->num_rows > 0) {
            ...
            if ($row['role'] === 'customer') {
                ...
                $errorMessage = "Access denied! Only customers can log in here.";
            }
        } else {
            $errorMessage = "Invalid username or password.";
        }
    } else {
        $errorMessage = "Server error: " . $conn->error; // Capture server-side error
    }
}
?>
...
<body>
    ...
    if (!empty($errorMessage)) {
        // Inject JavaScript alert for error messages
