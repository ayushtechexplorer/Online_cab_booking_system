...
$username = $password = $local_area = $gender = "";

// Insert new driver into the database if the form is submitted
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $insert_sql = "INSERT INTO users (username, password, role, local_area, gender) VALUES (?, ?, 'driver', ?, ?)";
    $stmt = $conn->prepare($insert_sql);
    $stmt->bind_param("ssss", $username, $password, $local_area, $gender);

    if ($stmt->execute()) {
        header("Location: manage_users.php"); // Redirect to manage users after successful insertion
        exit();
    } else {
        echo "Error adding new driver: " . $stmt->error;
    }
}
?>
